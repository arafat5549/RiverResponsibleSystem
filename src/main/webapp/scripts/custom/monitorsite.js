var TreeCl = function () {
     var ajaxTreeCl = function() {
        $("#tree_cl").jstree({
            ...
            "state" : { "key" : "monitorsite" },
            "plugins" : [ "state", "types" ]
        })
        .on('select_node.jstree', function (e, data) {
        	Monitorsite.load(data.node.id);
        })    
    }
    return {
	...
	return {
		load: function(id){
			...
			Cl.ajaxRequest(url,data,function(result){
				...
				$("#name").blur();
                $("#userId").val(node.userId).trigger("change");
				Cl.selected = node;
			});
		},
		//增加节点
