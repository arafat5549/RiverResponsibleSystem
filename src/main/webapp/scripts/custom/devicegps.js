	...
	return {
		load: function(id){
			...
			Cl.ajaxRequest(url,data,function(result){
                ...
                $("#monitorsiteId").val(node.monitorsiteId).trigger("change");
                $("#monitorsiteId").blur();
                $("#deviceinfoId").val(node.deviceinfoId).trigger("change");
                $("#deviceinfoId").blur();
				$("#longitude").attr("value",node.longitude);
				$("#longitude").blur();
                $("#latitude").attr("value",node.latitude);
                $("#latitude").blur();
                $("#ipAddress").attr("value",node.ipAddress);
                $("#ipAddress").blur();
                $("#port").attr("value",node.port);
                $("#port").blur();
		...
		add: function(){	
            ...
            if(!sel.length) {
                var status = confirm("您确定是要添加根节点么？");
                //alert(status);
                if(!status) {
                    alert('请选择一个节点！111');
                    return false;
            ...
            sel = sel == undefined ? undefined : sel[0];
			var newNodeText = $('#name').val();
			if(sel != undefined && newNodeText == sel.text){
				alert('子级名称不能与父级相同！');
				return false;
			}	
			var url="add.do";
			var data= {
				"monitorsiteId":$("#monitorsiteId").val(),
                "deviceinfoId": $("#deviceinfoId").val(),
                "longitude" : $("#longitude").val(),
                "latitude" : $("#latitude").val(),
                "ipAddress" : $("#ipAddress").val(),
                "port" : $("#port").val(),
                "parentId": function () {
					if(sel == undefined ||  isNaN(sel.id)){
						return 0;
					}else{
						return sel.id;
					}
			...
			Cl.ajaxRequest(url,data,function(result){
				...
				if(node.id){
					var nodeData = {
						id:node.id,
						text:node.id,
                        monitorsiteId:node.monitorsiteId,
                        deviceinfoId:node.deviceinfoId,
