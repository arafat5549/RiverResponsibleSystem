var DataTableCl = function () {
    var handleDeviceinfo = function(ol) {
        ...
        grid.init({
            ...
            dataTable: {  // here you can define a typical datatable settings from http://datatables.net/usage/options
                ...
                "sAjaxSource": "getDeviceinfoDataTables.do", // ajax source
                "fnServerData": function (sSource, aoData, fnCallback) {// 获取数据的处理函数
                    var json = myForm.serializeObject('searchForm');
                    json["iDisplayStart"] = function () {
                        if(myForm.arrayToObject(aoData).iDisplayStart=="" || myForm.arrayToObject(aoData).iDisplayStart==null){
                            return 0;
                        }
                        return myForm.arrayToObject(aoData).iDisplayStart;
                    };
                    json["iDisplayLength"] = function(){
                        if(myForm.arrayToObject(aoData).iDisplayLength=="" || myForm.arrayToObject(aoData).iDisplayLength==null){
                            return 10;
                        }
                        return myForm.arrayToObject(aoData).iDisplayLength;
                    };
                    $.ajax({
                        "dataType": 'json',
...
var Deviceinfo = function(){
	return {
         ...
         * 点击查看图片
         */
        image_click: function(id) {
            Cl.action='image';
            Cl.showModalWindow(Cl.modalName,"imageform.do?id="+id);
        },
		/**
		 * 增加角色
		 */
		add: function(){debugger
			alert(0)
			var url="add.do";
			var data=myForm.serializeObject("form_cl");
		...
		update: function(){
			...
			Cl.ajaxRequest(url,data,function(result){
				...
				if(result == "success"){			
					Cl.hideModalWindow(Cl.modalName);
					Cl.updateDataRow(DataTableCl.tableName,data.id,0,'getDeviceinfoDataRow.do');
					alert("修改成功");
				} else {
					alert("修改失败");
					return ;			
         ...
         * 删除设备
         */
        removeImage: function(a,id,name,deviceinfoId){
            if(!confirm("确定要删除该图片")){
                return;
            }
            var url="deleteImage.do";
            var data={
                "id":id,
				"src":name
            };
            Cl.ajaxRequest(url,data,function(result){
                ...
                if(result == "success"){
                   ...
                   // $(a).parent().parent().empty();//.remove();
                    Cl.hideModalWindow(Cl.modalName);
					Deviceinfo.image_click(deviceinfoId);
                    Cl.updateDataRow(DataTableCl.tableName,data.deviceinfoId,0,'getDeviceinfoDataRow.do');
                    alert("删除成功");
                } else {
                    alert("删除失败");
                    return ;
