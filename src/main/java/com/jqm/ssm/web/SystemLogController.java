...
public class SystemLogController {
    ...
    public BaseResult<Object> list(Integer offset, Integer limit) {
        ...
        try {
            ...
            list = systemLogService.listPage(offset, limit,null);
    ...
    @ResponseBody
    public BaseResult<Object> save(@Valid @RequestBody BaseRequestBody<SystemLog> body, BindingResult result, HttpServletRequest request) {
        User loginuser = SessionUtil.getSessionUser(request);
        SystemLog bean = null;
        int resultCode = -1;
        try {
            bean = body.getData();
            resultCode = systemLogService.insertSelective(bean);
    ...
    @ResponseBody
    public BaseResult<Object> delete(@Valid @RequestBody BaseRequestBody<SystemLog> body, BindingResult result, HttpServletRequest request) {
        SystemLog bean = null;
        int resultCode = -1;
        try {
            bean = body.getData();
            resultCode = systemLogService.deleteByPrimaryKey(bean.getId());
    ...
    @ResponseBody
    public BaseResult<Object> update(@Valid @RequestBody BaseRequestBody<SystemLog> body, BindingResult result, HttpServletRequest request){
        SystemLog bean = null;
        int resultCode = -1;
        try {
            bean = body.getData();
            resultCode = systemLogService.updateByPrimaryKeySelective(bean);
        } catch (BizException e) {
            return new BaseResult<Object>(false, e.getMessage());
        } catch (Exception e) {
            if(Constants.Debug()) e.printStackTrace();
    ...
    public BaseResult<Object> treedata(HttpServletRequest request, HttpServletResponse response) {
        ...
        String treedata = getSystemLogTree();
    ...
    public String main(String visitedModule, String visitedResource, HttpServletRequest request, ModelMap map) {
        ...
        map.put("user", user);
        map.put("menus", menus);
        return "systemlog/list.ftl";
    }

    @ResponseBody
    @RequestMapping("/getSystemLogDataTables")
    public String getSystemLogDataTables(SystemLogSearchModel searchModel, ModelMap map) {
        LOG.info(searchModel.getIDisplayStart()+","+searchModel.getIDisplayLength());
        return systemLogService.getSystemLogDataTables(searchModel);
    }

    @ResponseBody
    @RequestMapping("/export")
    public void exportToExcel(SystemLogSearchModel searchModel,HttpServletRequest request,HttpServletResponse response){
        List<SystemLog> systemLogs = systemLogService.getSystemLogListBySearch(searchModel);
        ExcelUtils.getListToExcel(systemLogs,"sheet","系统日志表",request,response,SystemLog.class);
    }
     ...
     * 获取treedata树型结构
     */
    public String getSystemLogTree(){
        return "{}";
    }
}
